...
#else /*linux freebsd*/
#define DEFAULT_CONFIG_FILE "/media/internal/dosbox.conf"
#endif
...
static SDLKey convertKeyWithMetaLocked(SDLKey key);

int metaKeyLocked(void); 
...
//			HandleVideoResize(&event.resize);
		...
		case SDL_KEYDOWN:
            if (
                ...
                if(clock_gettime(CLOCK_MONOTONIC, &tp) == 0) {
                    if(pressTime[metakey] == 0LL) {
                        pressTime[metakey] = (tp.tv_sec * 1000LL) + (tp.tv_nsec / 1000000LL);
                        keyInterval[metakey] = 0;
                    } else {
                        curTime = (tp.tv_sec * 1000LL) + (tp.tv_nsec / 1000000LL);
                        keyInterval[metakey] = curTime - pressTime[metakey];
                        pressTime[metakey] = curTime;
                    }

                    if(keyInterval[metakey] != 0 && (keyInterval[metakey] < LOCK_INTERVAL)) {
                        pressTime[metakey] = 0LL;

                        if(metaLocked[metakey] == false) {
                            metaLocked[metakey] = true;
                        } else {
                            metaLocked[metakey] = false;
                        }
		...
		case SDL_KEYUP:
		default:
            if(metaLocked[0] == true || metaLocked[1] == true) {
                event.key.keysym.sym = convertKeyWithMetaLocked(event.key.keysym.sym);
            }
...
static void erasemapperfile() {
	FILE* g = fopen(DEFAULT_CONFIG_FILE,"r");
	if(g) {
		fclose(g);
...
static SDLKey convertKeyWithMetaLocked(SDLKey key)
{
    ...
    switch(key) {
        /* DIGITS */
        case SDLK_AT: convKey = SDLK_0; break;
        default:
                      convKey = key;
    }
